function generateEventToggles(eventInfo, eventConfig, eventEnabled) {
  ...
  cardContent
    .append('<div class="field is-horizontal"')
    .append($('<h5>' + eventInfo['name'] + '</h5>'))
    .append($("<p style='font-size: 15px;'>" + eventInfo['desc'] + '</p>'));
  if (typeof eventInfo['info'] === 'string') {
    cardContent.append(
      $('<div class="field is-horizontal">')
        .append(
          $(
            '<div class="field-label is-normal"><label class="label" for="' +
              eventInfo['id'] +
              '">Enable</label></div>'
          )
        )
        .append(
          $(
            '<div class="field-body"><div class="field"><div class="control"><label class="switch is-rounded"><input type="checkbox" ' +
              (eventConfig['toggle'] ? 'checked' : '') +
              ' name="' +
              eventInfo['id'] +
              '"><span class="check"></span></label></div><p class="help">' +
              eventInfo['info'] +
              '</p></div></div>'
          )
        )
          ...
          $('<div class="field is-horizontal">')
            ...
            .append(
              $(
                '<div class="field-body"><div class="field"><div class="control"><label class="switch is-rounded"><input type="checkbox" ' +
                  (eventConfig['toggle'][
                    eventInfo['id'] + '_' + (parseInt(infoIter) + 1).toString()
                  ]
                    ? 'checked'
                    : '') +
                  ' name="' +
...
async function insertNewEventConfig(eventData, eventIter, eventID) {
  ...
  if (typeof eventData['events'][eventIter]['info'] !== 'string') {
    ...
    for (const toggleIter in eventData['events'][eventIter]['info']) {
      toggle[
        eventData['events'][eventIter]['id'] + '_' + (parseInt(toggleIter) + 1)
      ] = false;
    }
  }
  ...
  $('#event-submit').on('click', async function () {
    $.each($('span.check'), function (index, value) {
      for (const eventIter in eventData['events']) {
        ...
        if (
          eventData['events'][eventIter]['id'] ===
          $(value).parent().children('input').attr('name')
        ) {
          ...
          $(value)
            ...
            .children('input')
            .attr('name')
            .includes(eventData['events'][eventIter]['id'] + '_')
        ) {
          if (
    ...
    for (const eventIter in eventData['events']) {
      if (eventData['events'][eventIter]['id'] === $(selectClass).val()) {
        $('.' + listClasses[selectClass] + '.list').append(
          generateEventToggles(
            eventData['events'][eventIter],
            eventConfig[eventData['events'][eventIter]['id']],
            eventData['events'][eventIter]['enabled']
          )
        );
        $('div.main').append(
          $(
