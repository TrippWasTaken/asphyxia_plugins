...
$(document).ready(function () {
  $.getJSON('static/asset/json/music_db.json', function (json) {
    ...
    for (let mdata in music_db.mdb.music) {
      ...
      temp_data.songname = temp_data.songname.replace(
        /[龕釁驩曦齷骭齶彜罇雋鬻鬥鬆曩驫齲騫趁鬮盥隍頽餮黻蔕闃]/g,
        m => translate_table[m]
      );
      if ('distribution_date' in music_db.mdb.music[mdata]['info']) {
        temp_data.releasedate =
          music_db.mdb.music[mdata]['info']['distribution_date']['#text'];
      } else {
      ...
      temp_data.oth = '';
      if (
        music_db.mdb.music[mdata]['difficulty']['novice']['difnum']['#text'] !=
        0
      ) {
        temp_data.nov =
          music_db.mdb.music[mdata]['difficulty']['novice']['difnum']['#text'];
      ...
      if (
        music_db.mdb.music[mdata]['difficulty']['advanced']['difnum'][
          '#text'
        ] != 0
      ) {
        temp_data.adv =
      ...
      if (
        music_db.mdb.music[mdata]['difficulty']['exhaust']['difnum']['#text'] !=
        0
      ) {
        temp_data.exh =
          music_db.mdb.music[mdata]['difficulty']['exhaust']['difnum']['#text'];
      ...
      if (music_db.mdb.music[mdata]['info']['inf_ver']['#text'] != 0) {
        temp_data.oth =
          music_db.mdb.music[mdata]['difficulty']['infinite']['difnum'][
            '#text'
          ] +
          ' | ' +
          getInfDifficulty(
            music_db.mdb.music[mdata]['info']['inf_ver']['#text']
          );
      }
      if ('maximum' in music_db.mdb.music[mdata]['difficulty']) {
        if (
          music_db.mdb.music[mdata]['difficulty']['maximum']['difnum'][
            '#text'
          ] != 0
        ) {
          temp_data.mxm =
