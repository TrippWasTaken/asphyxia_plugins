...
$('#nemsys_select').change(function () {
  $('#nemsys_pre').fadeOut(200, () => {
    if ($('#nemsys_select').val() != 30) {
      $('#nemsys_pre').attr(
        'src',
        'static/asset/nemsys/nemsys_' +
          zeroPad($('#nemsys_select').val(), 4) +
          '.png'
      );
    } else {
...
$('[name="subbg"]').change(async function () {
  let subbgType = database['subbg'].filter(
    e => e.value === parseInt($('[name="subbg"]').val())
  )[0]['type'];
  let isSubbgSlideshow = subbgType === 'slideshow';
  $('#sub_pre').fadeOut(200);
  $('#sub_pre_vid').fadeOut(200);
  if (subbgType === 'video') {
    $('#sub_pre_vid_src').attr(
      'src',
      'static/asset/submonitor_bg/subbg_' +
        zeroPad($('[name="subbg"]').val(), 4) +
        '.mp4'
    );
      ...
      $('#sub_pre').attr(
        ...
        isSubbgSlideshow
          ? 'static/asset/submonitor_bg/subbg_' +
              ...
              getImageFileFormat(
                0,
                parseInt(zeroPad($('[name="subbg"]').val(), 4))
              )
          : 'static/asset/submonitor_bg/subbg_' +
              zeroPad($('[name="subbg"]').val(), 4) +
              getImageFileFormat(
...
$('[name="bgm"]').change(function () {
  $('#custom_0').attr(
    'src',
    'static/asset/audio/custom_' +
      zeroPad($('[name="bgm"]').val(), 2) +
      '/0.mp3'
  );
  $('#custom_1').attr(
    'src',
...
$('[name="stampA"]').change(function () {
  $('#a_pre').fadeOut(200, () => {
    ...
    if (stamp == 0) {
      ...
      $('#a_pre').attr(
        'src',
        'static/asset/chat_stamp/stamp_' +
          zeroPad(group, 4) +
          '/stamp_' +
          zeroPad(group, 4) +
          '_' +
...
$(document).ready(function () {
  ...
  $.getJSON('static/asset/json/customize_data_ext.json', function (json) {
    ...
    let bplSupport = profile_data['bplSupport']
      ? profile_data['bplSupport'] % 10
      : 0;
    $('[name="bplSupport"]').val(bplSupport);

    if (profile_data['bplSupport'] >= 10)
      $('[name="bplPro"]').attr('checked', true);

    for (var i in json['sysbg']) {
      if (
        unlock_all ||
        items_sysbg.find(x => x.id === json['sysbg'][i].id) ||
        json['sysbg'][i].id === 0
      ) {
        $('[name="sysBG"]').append(
          $('<option>', {
    ...
    for (var i in json['skilltitle']) {
      let foundCourses = courses.filter(
        c => c.cid === json['skilltitle'][i].id && c.clear >= 2
      );
      if (foundCourses.length > 0) {
        $('[name="skilltitle"]').append(
          $('<option>', {
            value: json['skilltitle'][i].id,
            text:
              json['skilltitle'][i].name +
              ' (' +
              json['skilltitle'][i].info +
              ')',
          })
        );
      }
    ...
    $('[name="creatorItem"]').val(
      profile_data['creatorItem'] ? profile_data['creatorItem'] : 0
    );
  });
  ...
  $.getJSON('static/asset/json/data.json', function (json) {
    ...
    for (var i in json['nemsys']) {
      if (
        ![8, 9, 10, 11].includes(json['nemsys'][i].value) &&
        (unlock_all ||
          json['nemsys'][i].value === 0 ||
          items_nemsys.find(x => x.id === json['nemsys'][i].value))
      ) {
        $('#nemsys_select').append(
          $('<option>', {
    ...
    for (var i in json['subbg']) {
      if (
        unlock_all ||
        json['subbg'][i].value === 0 ||
        items_subbg.find(x => x.id === json['subbg'][i].value)
      ) {
        $('[name="subbg"]').append(
          $('<option>', {
        ...
        if (json['subbg'][i].type === 'slideshow') {
          image.src =
            'static/asset/submonitor_bg/subbg_' +
            zeroPad(json['subbg'][i].value, 4) +
            '_0' +
            (Math.floor(Math.random() * 3) + 1) +
            getImageFileFormat(0, parseInt(zeroPad(json['subbg'][i].value, 4)));
    ...
    for (var i in json['bgm']) {
      if (
        unlock_all ||
        json['bgm'][i].value === 0 ||
        items_bgm.find(x => parseInt(x.id) === parseInt(json['bgm'][i].value))
      ) {
        $('[name="bgm"]').append(
          $('<option>', {
    ...
    for (var i in json['stamp']) {
      if (
        unlock_all ||
        json['stamp'][i].value === 0 ||
        items_stamp.find(x => x.id === json['stamp'][i].value)
      ) {
        $('[name="stampA"]').append(
          $('<option>', {
  ...
  $.getJSON('static/asset/json/data.json', function (json) {
    database = json;
    let subbgType = database['subbg'].filter(
      e => e.value === parseInt(profile_data['subbg'])
    )[0]['type'];
    let isSubbgSlideshow = subbgType === 'slideshow';
    ...
    if (subbgType === 'video') {
      $('#sub_pre_vid')
        ...
        .append(
          $(
            "<source id='sub_pre_vid_src' src='static/asset/submonitor_bg/subbg_" +
              zeroPad(profile_data['subbg'], 4) +
              ".mp4'>"
          )
        );
      $('#sub_pre_vid').fadeIn(200);
      ...
      $('#sub_pre').attr(
        ...
        isSubbgSlideshow
          ...
          : 'static/asset/submonitor_bg/subbg_' +
              zeroPad(profile_data['subbg'], 4) +
              getImageFileFormat(0, parseInt(zeroPad(profile_data['subbg'], 4)))
      );
      $('#sub_pre').fadeIn(200);
    }
  });
  ...
  $('#custom_0').attr(
    'src',
    'static/asset/audio/custom_' + zeroPad(profile_data['bgm'], 2) + '/0.mp3'
  );
  $('#custom_1').attr(
    'src',
    'static/asset/audio/custom_' + zeroPad(profile_data['bgm'], 2) + '/1.mp3'
  );
  $('#custom_0').prop('volume', 0.5);
  $('#custom_1').prop('volume', 0.2);
