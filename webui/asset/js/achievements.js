...
$(document).ready(async function () {
  ...
  achobt.sort(function (a, b) {
    return a['id'] - b['id'];
  });
  achdata = await $.getJSON(
    'static/asset/json/achievements.json',
    function (data) {
      return data;
    }
  );
  ...
  achdata['achievements'].forEach(ach => {
    ...
    if (ach['rid'] !== -1) {
      if (ach['rtype'] === 'pcb') {
        ...
        let aptitle = aptitlelist.find(
          e => parseInt(e['value']) === ach['rid']
        );
  ...
  $('#achlist').DataTable({
    ...
    columns:
      listType === 3
        ...
        : [{ data: 'id' }, { data: 'title' }, { data: 'reward' }],
    pageLength: 100,
    columnDefs: [
      {
        targets: 0,
