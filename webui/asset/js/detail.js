...
function getSkillAsset(skill) {
  return (
    "static/asset/skill_lv/skill_" +
    (skill === -1 ? "none" : zeroPad(skill, 2)) +
    ".png"
  )
}
...
function getSkillTitle() {
  if (skill_data.length <= 0 || skill_data[0].name === (undefined || 0))
    return ""
...
function getAppealCard(appeal) {
  var result = appeal_db["appeal_card_data"]["card"].filter(
    (object) => object["@id"] == appeal
  )
  return (
    "static/asset/ap_card/" +
    (result.length > 0 ? result[0]["info"]["texture"] : "ap_06_0001") +
    ".png"
  )
}
...
function setUpStatistics() {
  ...
  score_db.forEach(function (currentValue, index, array) {
    ...
    CMpLArray[parseInt(getSongLevel(currentValue.mid, currentValue.type)) - 1][
      currentValue.clear - 1
    ] += 1
    GpDArray[currentValue.type][currentValue.grade - 1]++
    GpLArray[parseInt(getSongLevel(currentValue.mid, currentValue.type)) - 1][
      currentValue.grade - 1
    ] += 1
    ASpLArray[
      parseInt(getSongLevel(currentValue.mid, currentValue.type)) - 1
    ][0] += 1
    ASpLArray[
      parseInt(getSongLevel(currentValue.mid, currentValue.type)) - 1
  ...
  for (var lv = 1; lv <= 20; lv++) {
    baseTBodyGpL.append(
      $("<tr>")
        .append($("<th>").append(lv))
        .append($("<td>").append(GpLArray[lv - 1][0]))
        .append($("<td>").append(GpLArray[lv - 1][1]))
        .append($("<td>").append(GpLArray[lv - 1][2]))
        .append($("<td>").append(GpLArray[lv - 1][3]))
        .append($("<td>").append(GpLArray[lv - 1][4]))
        .append($("<td>").append(GpLArray[lv - 1][5]))
        .append($("<td>").append(GpLArray[lv - 1][6]))
        .append($("<td>").append(GpLArray[lv - 1][7]))
        .append($("<td>").append(GpLArray[lv - 1][8]))
        .append($("<td>").append(GpLArray[lv - 1][9]))
    )
  }
  for (var lv = 1; lv <= 20; lv++) {
    baseTBodyASpL.append(
      $("<tr>")
        .append($("<th>").append(lv))
        .append($("<td>").append(getAvg(ASpLArray, lv)))
    )
  }
}
...
$("#version_select").change(function () {
  $("#skillLV").fadeOut(200, () => {
    ...
    $("#skillLV").attr(
      "src",
      getSkillAsset(getPlayerSkill($("#version_select").val())[0])
    )
  })
  $("#skillLV").fadeIn(200)
})
...
function getPlayerSkill(version) {
  ...
  let courseDataFil = course_data.filter(
    (e) =>
      ![6, 7, 12, 13, 15, 16].includes(e.sid) &&
      e.cid === k[0].level &&
      (k[0].type !== undefined ? k[0].type : 0) ===
        (e.stype !== undefined ? e.stype : 0)
  )
  return [
    courseDataFil.length > 0 ? k[0].level : -1,
    k[0].type !== undefined ? k[0].type : 0,
  ]
}
...
function getPlayerCourse(version, playerSkill) {
  ...
  sidCourses = sidCourses.filter(
    (e) => ![6, 7, 12, 13, 15, 16].includes(e.sid) && e.level === playerSkill[0]
  )
  let newCourses = sidCourses.filter((e) => e.isNew === 1).map((a) => a.sid)
  let foundCourses = course_data.filter(
    (e) =>
      e.cid === playerSkill[0] &&
      (e.stype !== undefined ? e.stype : 0) === playerSkill[1] &&
      e.clear >= 2
  )
  let newCompleteCourses = foundCourses.filter((e) =>
    newCourses.includes(e.sid)
  )
  let thrshCourses = foundCourses.filter(
    (e) => Math.floor(e.rate / 100) >= skillThrsh[playerSkill[1]][skillThrshVal]
  )

  if (sidCourses.length > 0 && thrshCourses.length === sidCourses.length)
    return skillFrame[playerSkill[1]] + "_sp"
  if (sidCourses.length > 0 && foundCourses.length === sidCourses.length)
    return skillFrame[playerSkill[1]] + "_gold"
  if (newCompleteCourses.length > 1)
    return skillFrame[playerSkill[1]] + "_silver"
  if (foundCourses.length > 1 && playerSkill[1] === 1)
    return skillFrame[playerSkill[1]] + "_sp_none"
  return "none"
}
  ...
  if (sznData !== undefined) {
      ...
      .append(
        $('<div class="tile is-parent is-5" style="position:relative">').append(
          $('<article class="tile is-child arena-details-child-right">').append(
            $(
              '<p class="title" style="font-family: testfont">Arena Power</p>'
            ).append(
    ...
    if (sznData.rankPoint >= 23600) {
      $(".arena-details-child-right").append(
        $(
          '<p class="title" style="font-family: testfont">Rank Point</p>'
        ).append(
    ...
    if (sznData.rankPoint !== 0 && sznData.rankPoint < 27800) {
      $(".arena-details-child-left")
        .append(
          $(
            '<meter id="rank-point-mtr" style="width:80%" min="' +
              rankInfo[1] +
              '"max="' +
              (rankInfo[2] !== undefined ? rankInfo[2].point : rankInfo[1]) +
              '" value="' +
              sznData.rankPoint +
              '"></meter>'
          )
        ...
        .append(
          $(
            '<h5 style="font-family: testfont">' +
              sznData.rankPoint +
              " pts (" +
              (rankInfo[2].point - sznData.rankPoint) +
              " pts to " +
              rankInfo[2].rank.toUpperCase() +
              ")</h5>"
          )
        )
        .append(
          sznData.megamixRate !== (0 || undefined)
            ? $(
                '<h5 style="font-family: testfont">Megamix rate: ' +
                  sznData.megamixRate +
                  ' <img style="width:20px" src="static/asset/arena_rank/mixstar.png"></p></div></h5>'
              )
            : $("<h5>")
        )
      ...
      $(".arena-details-child-left").append(
        sznData.megamixRate !== (0 || undefined)
          ...
          : $(
              '<h5 style="font-family:testfont">Ultimate rate: ' +
                sznData.ultimateRate +
                "</h5>"
            )
      )
    }
...
$(document).ready(function () {
  ...
  $.when(
    ...
    $("#test")
      .append(
        $('<div class="card is-inlineblocked" style="padding-bottom:30px">')
          .append(
                  ...
                  .append(
                    $('<div class="tile is-parent is-3">').append(
                      $('<article class="tile is-child">')
                        .append(
                          $("<img>")
                            .attr("src", getAppealCard(profile_data.appeal))
                            .css("width", "150px")
                        )
                    ...
                    $('<div class="tile is-parent is-6">').append(
                      $('<article class="tile is-child">').append(
                        $("<div>")
                          ...
                          .append(
                            $("<div>")
                              .append(getAkaname(profile_data["akaname"]))
                              .css("font-size", "35px")
                          )
                    ...
                    $('<article class="tile is-child">').append(
                      $(
                        ...
                        .append(
                          $('<img id="skillLV">').attr(
                            "src",
                            getSkillAsset(getPlayerSkill(maxVer)[0])
                          )
                        )
                        .append(
                          $(
                        ...
                        .append(
                          $(
                            '<div style="position:absolute;padding-left:20px;width:100%;height:100%;display: flex;justify-content: center;align-items: center;">'
                          ).append($('<p id="skillTitle"></p>'))
                        )
                    )
                  )
                )
                  ...
                  $('<div class="tile is-parent is-5">').append(
                    $('<article class="tile is-child">').append(
                      $('<p class="title">')
                        .append("PCB")
                        .append(
                          $('<div class="content">').append(profile_data.blocks)
                        )
                        .css("font-family", "testfont")
                    )
                  )
      ...
      .append(
        $('<div class="card  is-inlineblocked">')
          .append(
            $('<div class="card-header">').append(
              $('<p class="card-header-title">')
                .append(
        ...
        $('<div class="card">')
          ...
          .append(
            $('<div class="card-content">')
              .append(
                $(
                  '<div class="tabs is-toggle is-paddingless is-centered is-fullwidth">'
                ).append(
                  $('<ul class="is-marginless">')
                    ...
                    .append(
                      $('<li name="gpl">').append(
                        $('<a onclick="setGpL()">').append("Grade per Level")
                      )
                    )
                    .append(
                      $('<li name="aspl">').append(
    ...
    arena_data = arena_data.sort(function (a, b) {
    ...
    if (skillFrame !== "none")
      $("#skillFrame").attr("src", getSkillFrameAsset(skillFrame))
    $("#skillTitle").text(getSkillTitle())
    $("#skillTitle").attr(
      "style",
      "font-size:25px; color:black; font-weight:bold;color:" +
        (getPlayerSkill(maxVer)[0] === 11
          ? "#FFC100"
          : getPlayerSkill(maxVer)[0] === 12
          ? "#FFE000"
          : "black")
    )
