function generateEventToggles(eventInfo, eventConfig) {
  ...
  cardContent
    ...
    .append($('<h5>' + eventInfo['key'] + '</h5>'))
    .append($("<p style='font-size: 15px;'>" + eventInfo['explain'] + '</p>'));
      ...
      .append(
        $(
          '<div class="field-body"><div class="field"><div class="control"><label class="switch is-rounded"><input type="checkbox" ' +
            ...
            ' name="' +
            eventInfo['id'] +
            '"><span class="check"></span></label></div><p class="help"></p></div></div>'
        )
      )
  );
  return cardContent;
...
$(document).ready(async function () {
  ...
  for (const flagIter in eventData['flags']) {
    if (flagConfig[eventData['flags'][flagIter]['id']] === undefined)
      flagConfig[eventData['flags'][flagIter]['id']] = {
        toggle: true,
      };
    $('.card').append(
      generateEventToggles(
        eventData['flags'][flagIter],
        flagConfig[eventData['flags'][flagIter]['id']]
      )
    );
  }
