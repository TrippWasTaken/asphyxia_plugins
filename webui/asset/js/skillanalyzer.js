...
$(document).ready(function () {
  ...
  $('#sdvx-version').change(function () {
    ...
    if (this.value != '-- Select --') {
      ...
      versionCourseData = courseData.courseData.find(
        data => parseInt(data['version']) === selectedVersion
      ).info;
      courseLevels = [];
      specificCourse = [];
      versionCourseData.forEach(function (item, index) {
        item.courses.forEach(function (item2, index2) {
          if (!courseLevels.some(course => course.includes(item2['name'])))
            courseLevels.push([item2['name'], item2['id']]);
        });
      });

      $.each(courseLevels, function () {
        $('#skill-course').append(
          '<option value=' + this[1] + '>' + this[0] + '</>'
        );
      });
    }
  });
  ...
  $('#skill-course').change(function () {
    ...
    if (this.value != '-- Select --') {
      ...
      versionCourseData.forEach(function (item, index) {
        item.courses.forEach(function (item2, index2) {
          if (
            !specificCourse.some(course => course.includes(item['name'])) &&
            item2['id'] == selectedCourseLevel
          ) {
            console.log(
              item['name'] +
                ' included in ' +
  ...
  $('#skill-course-level').change(function () {
    if (this.value != '-- Select --') {
      ...
      theCourse = versionCourseData
        .find(data => data.id === selectedSpecificCourse)
        .courses.find(data => data.id === selectedCourseLevel).tracks;
      trackIndex = 0;
      difficultyLabels = [
        'Novice',
        'Advanced',
      ...
      theCourse.forEach(function (courseTrack) {
        track = musicDB.mdb.music.find(
          data => parseInt(courseTrack.mid) === parseInt(data['@id'])
        );
        ...
        $('.track-difficulty')
          ...
          .append(
            '<option value=' +
              ...
              difficultyLabels[0] +
              ' (' +
              track.difficulty['novice'].difnum['#text'] +
              ')</option>'
          );
        $('.track-difficulty')
          .eq(trackIndex)
          .append(
            '<option value=' +
              ...
              difficultyLabels[1] +
              ' (' +
              track.difficulty['advanced'].difnum['#text'] +
              ')</option>'
          );
        $('.track-difficulty')
          .eq(trackIndex)
          .append(
            '<option value=' +
              ...
              difficultyLabels[2] +
              ' (' +
              track.difficulty['exhaust'].difnum['#text'] +
              ')</option>'
          );
        if (
          track.difficulty['maximum'].difnum['#text'] != '0' ||
          track.difficulty['infinite'].difnum['#text'] != '0'
        ) {
          $('.track-difficulty')
            .eq(trackIndex)
            .append(
              '<option value=' +
                (courseTrack.mty == 3 ? 3 : 4) +
                '>' +
                (track.difficulty['infinite'].difnum['#text'] != '0'
                  ? infLabel[parseInt(track.info.inf_ver['#text'])]
                  : difficultyLabels[4]) +
                ' (' +
                (track.difficulty['infinite'].difnum['#text'] != '0'
                  ? track.difficulty['infinite'].difnum['#text']
                  : track.difficulty['maximum'].difnum['#text']) +
                ')</option>'
            );
        }
  ...
  $('.track-id').on('input', function () {
    ...
    track = musicDB.mdb.music.find(
      data => searchTrackID === parseInt(data['@id'])
    );
    if (track) {
      ...
      if (
        ...
        $('.track-difficulty')
          ...
          .append(
            '<option value=' +
              (parseInt(track.info.inf_ver['#text']) != 0 ? 3 : 4) +
              '>' +
              (track.difficulty['infinite'].difnum['#text'] != '0'
                ? infLabel[parseInt(track.info.inf_ver['#text'])]
                : difficultyLabels[4]) +
