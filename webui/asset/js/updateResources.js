...
$(document).ready(async function () {
  ...
  $('#updateResources').click(async function () {
    answer = confirm(
      'Clicking OK would mean that you have already updated the datacode in your ea3-config.xml file. Would you like to proceed?'
    );
    if (answer == true) {
      document.getElementById('logtextarea').textContent = '';
      document.getElementById('logtextarea').textContent = 'Running....\n';
      await emit('copyResourcesFromGame').then(function (response) {
        ...
        if (response['data']['course']) {
          document.getElementById('logtextarea').textContent +=
            'Updated course_data.json from data/exg.ts!';
        }
        document.getElementById('logtextarea').textContent += '\n\n\n';

        if (response['data']['ifs'].length > 0) {
          document.getElementById('logtextarea').textContent +=
            'Successfully extracted textures: \n';
        ...
        if (response['data']['versionSongs'].length > 0) {
          document.getElementById('logtextarea').textContent +=
            'New songs in latest version data: \n';
