...
export const copyResourcesFromGame = async (data: {}, send: WebUISend) => {
    ...
    if (sig === magic) {
      ...
      if (md5_hash === textureslist[listIter].md5) {
        for (
          ...
          let imgBufferOff = 0;
          let imgUncompressedSize =
            imgBufferHead.readUInt32BE(imgBufferHeadOff);
          imgBufferHeadOff += 4;
  ...
  try {
    const files = await recursive(
      U.GetConfig('sdvx_eg_root_dir') + '/data/music'
    );
    const distDir = 'plugins/sdvx@asphyxia/webui/asset/jacket/';
    const filteredFiles = files.filter(file => /jk_\d{4}_1_s\.png$/.test(file));
...
export const addRival = async (
  ...
  let rival = await DB.FindOne<Profile>(data.rivalId, {
    collection: 'profile',
  });
    ...
    while (!finishedRolling) {
      ...
      if (unobtainedItems.length > 0) {
        let randomItemIndex = Math.floor(
          Math.random() * unobtainedItems.length
        );
        console.log(
          'Rolled item id: ' +
