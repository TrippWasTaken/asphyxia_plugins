...
export const updateProfile = async (data: {
  ...
  if (data.bplSupport && data.bplSupport.length > 0) {
    const validBplSupport = data.bplPro
      ? parseInt(data.bplSupport) + 10
      : parseInt(data.bplSupport)
...
export const updateMix = async (data: {
  ...
  await DB.Update<Mix>({ collection: "mix", code: data.code }, { $set: update })
}
...
export const copyResourcesFromGame = async (data: {}, send: WebUISend) => {
  ...
  let runErrors = []
  let newJacketData = []
  let resourceJsonData = JSON.parse(
    U.DecodeString(await IO.ReadFile("webui/asset/json/data.json"), "utf8")
  )
  let apCardJsonData = JSON.parse(
    U.DecodeString(await IO.ReadFile("webui/asset/json/appeal.json"), "utf8")
  )

  // Get new music data from music_db.xml
  console.log("Getting new music_db info")
  if (
    IO.Exists(U.GetConfig("sdvx_eg_root_dir") + "/data/others/music_db.xml")
  ) {
    let ea3Config = []
    let version = ""
    let mdb = U.parseXML(
      U.DecodeString(
        await IO.ReadFile(
          U.GetConfig("sdvx_eg_root_dir") + "/data/others/music_db.xml"
        ),
        "shift_jis"
      ),
      false
    ...
    if (IO.Exists(U.GetConfig("sdvx_eg_root_dir") + "/prop/ea3-config.xml")) {
      ...
      ea3Config = U.parseXML(
        U.DecodeString(
          await IO.ReadFile(
            U.GetConfig("sdvx_eg_root_dir") + "/prop/ea3-config.xml"
          ),
          "shift_jis"
        ),
        false
      ...
      IO.Exists(U.GetConfig("sdvx_eg_root_dir") + "/prop/ea3-ident.xml")
    ) {
      console.log("Reading ea3-ident.xml")
      ea3Config = U.parseXML(
        U.DecodeString(
          await IO.ReadFile(
            U.GetConfig("sdvx_eg_root_dir") + "/prop/ea3-ident.xml"
          ),
          "shift_jis"
        ),
        false
    ...
    //   prevAssetMdb = JSON.parse(
    //     U.DecodeString(
    //       await IO.ReadFile("webui/asset/json/music_db.json"),
    //       "utf8"
    //     )
    //   )
    // }
    mdb.mdb.music.forEach((musicValue) => {
      ...
      const folderId = musicValue["@attr"].id.padStart(4, "0")



      // if (
      //   IO.Exists(U.GetConfig("sdvx_eg_root_dir") + "/data/music")
      // ) {
      //   let subBGFiles = await IO.ReadDir(
      //     U.GetConfig("sdvx_eg_root_dir") + "/data/graphics/submonitor_bg"
      //   )
      //   for await (const subbg of subBGFiles) {
      //     if (subbg.name.match(/^(subbg[_0-9]*)(\.png|\.jpg|\.mp4)/g)) {
      //       let fileToWrite = await IO.ReadFile(
      //         U.GetConfig("sdvx_eg_root_dir") +
      //           "/data/graphics/submonitor_bg/" +
      //           subbg.name
      //       )
      //     }
      //   }
      ...
      if (Object.keys(prevAssetMdb).length > 0) {
        if (
          prevAssetMdb["mdb"]["music"].find(
            (item) => parseInt(item["@id"]) == parseInt(musicValue["@attr"].id)
          ) == undefined
        ) {
          console.log(
            "New song added to json: " +
              musicValue.info.title_name["@content"] +
              " (" +
              musicValue.info.distribution_date["@content"] +
              ")"
          )
          newJsonSongs.push([
            musicValue["@attr"].id,
        ...
        if (
          prevAssetMdb["mdb"]["music"].find(
            (item) =>
              parseInt(item["@id"]) == parseInt(musicValue["@attr"].id) &&
              parseInt(item["info"]["inf_ver"]["#text"]) === 0
          ) != undefined
        ) {
          if (musicValue.info.inf_ver["@content"] == "6") {
            console.log(
              "New XCD difficulty song: " +
                musicValue.info.title_name["@content"] +
                " (" +
                musicValue.info.distribution_date["@content"] +
                ")"
      ...
      if (
        parseInt(musicValue.info.distribution_date["@content"][0]) >=
        parseInt(version.substring(0, 8))
      ) {
        console.log(
          "Found new song for version " +
            version +
            ": " +
            musicValue.info.title_name["@content"] +
            " (" +
      ...
      mdbJsonFix.push({
        ...
        info: {
          ...
          artist_name: musicValue.info.artist_name["@content"],
          version: {
            "@__type": "u8",
        ...
        difficulty: {
          novice: {
            difnum: {
              "@__type": "u8",
              "#text":
                musicValue.difficulty.novice.difnum["@content"][0].toString(),
            },
          },
          advanced: {
            difnum: {
              "@__type": "u8",
              "#text":
                musicValue.difficulty.advanced.difnum["@content"][0].toString(),
            },
          },
          exhaust: {
            difnum: {
              "@__type": "u8",
              "#text":
                musicValue.difficulty.exhaust.difnum["@content"][0].toString(),
            },
          },
          maximum: {
            difnum: {
              "@__type": "u8",
              "#text":
                musicValue.difficulty.maximum != undefined
                  ? musicValue.difficulty.maximum.difnum[
                      "@content"
                    ][0].toString()
                  : "0",
            },
          ...
          infinite: {
            difnum: {
              "@__type": "u8",
              "#text":
                musicValue.difficulty.infinite != undefined
                  ? musicValue.difficulty.infinite.difnum[
                      "@content"
                    ][0].toString()
                  : "0",
            },
    ...
    IO.WriteFile(
      "webui/asset/json/music_db.json",
      JSON.stringify(mdbJsonFixFinal, null, 4)
    )
  } else {
    console.log(
      'Error reading music_db.xml. Check your "Exceed Gear Data Directory" config.'
    )
    runErrors.push(
      '[music_db] Error reading music_db.xml. Check your "Exceed Gear Data Directory" config.'
    )
  }
  ...
  if (
    IO.Exists(U.GetConfig("sdvx_eg_root_dir") + "/data/graphics/game_nemsys")
  ) {
    let nemsysFiles = await IO.ReadDir(
      U.GetConfig("sdvx_eg_root_dir") + "/data/graphics/game_nemsys"
    )
    for await (const nemsys of nemsysFiles) {
      let fileToWrite = await IO.ReadFile(
        U.GetConfig("sdvx_eg_root_dir") +
          "/data/graphics/game_nemsys/" +
          nemsys.name
      )
      if (
        !IO.Exists(
          "webui/asset/nemsys/" +
            nemsys.name.substring(0, nemsys.name.length - 4) +
            ".png"
        ) &&
        !IO.Exists(
          "webui/asset/nemsys/" +
      ...
      if (nemsys.name.match(/([0-9]+)/g) != undefined) {
        ...
        if (
          nemsysId &&
          ![8, 9, 10, 11].includes(nemsysId) &&
          resourceJsonData.nemsys.find((nem) => nem.value == nemsysId) ==
            undefined
        ) {
    ...
    console.log(
      'Error reading nemsys directory. Check your "Exceed Gear Data Directory" config.'
    )
    runErrors.push(
      '[nemsys] Error reading nemsys directory. Check your "Exceed Gear Data Directory" config.'
    )
  }
  ...
  if (
    IO.Exists(U.GetConfig("sdvx_eg_root_dir") + "/data/graphics/submonitor_bg")
  ) {
    let subBGFiles = await IO.ReadDir(
      U.GetConfig("sdvx_eg_root_dir") + "/data/graphics/submonitor_bg"
    )
    for await (const subbg of subBGFiles) {
      if (subbg.name.match(/^(subbg[_0-9]*)(\.png|\.jpg|\.mp4)/g)) {
        ...
        if (
          !IO.Exists(
            "webui/asset/submonitor_bg/" +
              subbg.name.substring(0, subbg.name.length - 4) +
              ".png"
          ) &&
          !IO.Exists(
            "webui/asset/submonitor_bg/" +
        ...
        if (subbgId) {
          ...
          let foundSubbg = resourceJsonData.subbg.findIndex(
            (subbg) => subbg.value === subbgId
          )
    ...
    console.log(
      'Error reading submonitor_bg directory. Check your "Exceed Gear Data Directory" config.'
    )
    runErrors.push(
      '[submonitor_bg] Error reading submonitor_bg directory. Check your "Exceed Gear Data Directory" config.'
    )
  }
  ...
  if (IO.Exists(U.GetConfig("sdvx_eg_root_dir") + "/data/sound/custom")) {
    let bgmFiles = await IO.ReadDir(
      U.GetConfig("sdvx_eg_root_dir") + "/data/sound/custom"
    )
      ...
      if (bgm.name.match(/(\.s3p)/g)) {
        ...
        if (folderName != "") {
          if (!IO.Exists("webui/asset/audio/" + folderName)) {
            ...
            let bufOffset = 4
            let s3pBuffer = await IO.ReadFile(
              U.GetConfig("sdvx_eg_root_dir") +
                "/data/sound/custom/" +
                bgm.name,
              { flag: "r" }
            ...
            entries.forEach((entry) => {
              let magic = s3pBuffer.toString(
                "utf8",
                entry.offset,
                entry.offset + 4
              )
              let fileStart = s3pBuffer.readInt32LE(entry.offset + 4)
              let arrayBuffer = s3pBuffer.buffer.slice(
                entry.offset,
                entry.offset + entry.length
              )
              IO.WriteFile(
                "webui/asset/audio/" + folderName + "/" + filename + ".wma",
                s3pBuffer.toString(
                  "binary",
                  entry.offset + fileStart,
                  entry.offset + entry.length
                ),
                { encoding: "binary" }
              )
          ...
          let bgmId = parseInt(
            bgm.name.match(/(?<=(custom|special)_)([0-9]*)/g)[0]
          )
          if (
            bgmId &&
            resourceJsonData.bgm.find((bgm) => bgm.value == bgmId) == undefined
          ) {
    ...
    console.log(
      'Error reading BGM directory. Check your "Exceed Gear Data Directory" config.'
    )
    runErrors.push(
      '[BGM] Error reading BGM directory. Check your "Exceed Gear Data Directory" config.'
    )
  }
  ...
  if (
    IO.Exists(U.GetConfig("sdvx_eg_root_dir") + "/data/others/chat_stamp.xml")
  ) {
    let chatStampData = U.parseXML(
      U.DecodeString(
        await IO.ReadFile(
          U.GetConfig("sdvx_eg_root_dir") + "/data/others/chat_stamp.xml"
        ),
        "shift_jis"
      ),
      false
    ...
    for (const chatStamp of chatStampData.chat_stamp_data.info) {
      if (
        resourceJsonData.stamp.find(
          (stamp) => stamp["value"] === chatStamp.id["@content"][0]
        ) == undefined
      ) {
        let stampTitle =
          chatStamp.title["@content"] +
          " " +
          (parseInt(chatStamp.id["@content"][0]) % 4 !== 0
            ? parseInt(chatStamp.id["@content"][0]) % 4
            : 4)
        console.log(
          "[chat_stamp] " + chatStamp.id["@content"][0] + " - " + stampTitle
        )
        resourceJsonData.stamp.push({
          value: chatStamp.id["@content"][0],
          name: stampTitle,
        })
        newChatStampData.push(
          chatStamp.id["@content"][0] + ": " + chatStamp.filename["@content"]
        )
      }
      if (
        !IO.Exists(
          "webui/asset/chat_stamp/" + chatStamp.filename["@content"] + ".png"
        ) &&
        !IO.Exists(
          "webui/asset/chat_stamp/" + chatStamp.filename["@content"] + ".png"
        )
        ...
        console.log(
          "[chat_stamp] copying " + chatStamp.filename["@content"] + ".png"
        )
        let fileToWrite = await IO.ReadFile(
          U.GetConfig("sdvx_eg_root_dir") +
            "/data/graphics/chat_stamp/" +
            chatStamp.filename["@content"] +
            ".png"
        )
        IO.WriteFile(
          "webui/asset/chat_stamp/" + chatStamp.filename["@content"] + ".png",
    ...
    resourceJsonData.stamp.sort(function (a, b) {
      return a.value - b.value
    })
  } else {
    console.log(
      'Error reading chat stamp xml file. Check your "Exceed Gear Data Directory" config.'
    )
    runErrors.push(
      '[chat stamp] Error reading chat stamp xml file. Check your "Exceed Gear Data Directory" config.'
    )
  }
  ...
  if (
    IO.Exists(U.GetConfig("sdvx_eg_root_dir") + "/data/graphics/valgene_item")
  ) {
    let valgeneItemFiles = await IO.ReadDir(
      U.GetConfig("sdvx_eg_root_dir") + "/data/graphics/valgene_item"
    )
    for await (const valgeneItem of valgeneItemFiles) {
      if (
        valgeneItem.name
          .substring(valgeneItem.name.length - 4, valgeneItem.name.length)
          .match(/(\.png|\.jpg)/g)
      ) {
        let fileToWrite = await IO.ReadFile(
          U.GetConfig("sdvx_eg_root_dir") +
            "/data/graphics/valgene_item/" +
            valgeneItem.name
        )
        if (
          !IO.Exists(
            "webui/asset/valgene_item/" +
              valgeneItem.name.substring(0, valgeneItem.name.length - 4) +
              ".png"
          ) &&
          !IO.Exists(
            "webui/asset/valgene_item/" +
          ...
          IO.WriteFile(
            "webui/asset/valgene_item/" + valgeneItem.name,
            fileToWrite
          )
    ...
    console.log(
      'Error reading valgene_item directory. Check your "Exceed Gear Data Directory" config.'
    )
    runErrors.push(
      '[valgene_item] Error reading valgene_item directory. Check your "Exceed Gear Data Directory" config.'
    )
  }
  ...
  if (
    IO.Exists(
      U.GetConfig("sdvx_eg_root_dir") + "/data/others/akaname_parts.xml"
    )
  ) {
    let akanameData = U.parseXML(
      U.DecodeString(
        await IO.ReadFile(
          U.GetConfig("sdvx_eg_root_dir") + "/data/others/akaname_parts.xml"
        ),
    ...
    for (const akaname of akanameData.akaname_parts.part) {
      if (
        resourceJsonData.akaname.find(
          (aka) => aka.value === akaname["@attr"].id
        ) === undefined
      ) {
        let akanameFmtd =
          "@content" in akaname.word
            ? akaname.word["@content"].replace(/(\[[A-z0-9:,\/\]]*)/g, "")
            : ""
        console.log(
          "[appeal title] adding " + akaname["@attr"].id + " - " + akanameFmtd
        )
        resourceJsonData.akaname.push({
          value: akaname["@attr"].id,
          name: akanameFmtd,
        })
        newAkanames.push(akaname["@attr"].id + ": " + akanameFmtd)
      }
    }
    resourceJsonData.akaname.sort(function (a, b) {
      return parseInt(a.value) - parseInt(b.value)
    })
  } else {
    console.log(
      'Error reading akaname xml file. Check your "Exceed Gear Data Directory" config.'
    )
    runErrors.push(
      '[akaname] Error reading akaname xml file. Check your "Exceed Gear Data Directory" config.'
    )
  }
  ...
  if (
    IO.Exists(U.GetConfig("sdvx_eg_root_dir") + "/data/others/appeal_card.xml")
  ) {
    let apCardData = U.parseXML(
      U.DecodeString(
        await IO.ReadFile(
          U.GetConfig("sdvx_eg_root_dir") + "/data/others/appeal_card.xml"
        ),
        "shift_jis"
      ),
      false
    ...
    for (const apCard of apCardData.appeal_card_data.card) {
      if (
        apCardJsonData.appeal_card_data.card.find(
          (ap) => ap["@id"] === apCard["@attr"].id
        ) == undefined
      ) {
        console.log(
          "[ap_card] adding to json: " +
            apCard["@attr"].id +
            " - " +
            apCard.info["title"]["@content"]
        )
        apCardJsonData.appeal_card_data.card.push({
          "@id": apCard["@attr"].id,
          info: {
            texture: apCard.info["texture"]["@content"],
            title: apCard.info["title"]["@content"],
          },
        })
        newAPCardData.push(
          apCard["@attr"].id +
            ": " +
            apCard.info["texture"]["@content"] +
            "(" +
            apCard.info["title"]["@content"] +
            ")"
        )
      ...
      if (
        !IO.Exists(
          "webui/asset/ap_card/" + apCard.info["texture"]["@content"] + ".png"
        ) &&
        !IO.Exists(
          "webui/asset/ap_card/" + apCard.info["texture"]["@content"] + ".jpg"
        )
      ) {
        console.log(
          "[ap_card] copying " + apCard.info["texture"]["@content"] + ".png"
        )
        let fileToWrite = await IO.ReadFile(
          U.GetConfig("sdvx_eg_root_dir") +
            "/data/graphics/ap_card/" +
            apCard.info["texture"]["@content"] +
            ".png"
        )
        IO.WriteFile(
    ...
    apCardJsonData.appeal_card_data.card.sort(function (a, b) {
      return parseInt(a["@id"]) - parseInt(b["@id"])
    })
  } else {
    console.log(
      'Error reading appeal card xml file. Check your "Exceed Gear Data Directory" config.'
    )
    runErrors.push(
      '[appeal card] Error reading appeal card xml file. Check your "Exceed Gear Data Directory" config.'
    )
  }

  await IO.WriteFile(
    "webui/asset/json/data.json",
    JSON.stringify(resourceJsonData, null, 4)
  )
  await IO.WriteFile(
    "webui/asset/json/appeal.json",
    JSON.stringify(apCardJsonData, null, 4)
  )
  ...
  //   let ifsBuffer = await IO.ReadFile(
  //     U.GetConfig("sdvx_eg_root_dir") + textureslist[listIter].file,
  //     { flag: "r" }
  //   )
  //   if (
  //     !fs.existsSync(
  //       "plugins/sdvx@asphyxia/webui/asset/" +
  //         textureslist[listIter].asset_folder
  //     )
  //   ) {
  //     fs.mkdirSync(
  //       "plugins/sdvx@asphyxia/webui/asset/" +
  ...
  //       for (
  //         let texIter = 0;
  //         texIter < textureslist[listIter]["textures"].length;
  //         texIter++
  //       ) {
  //         let tdFileName = textureslist[listIter]["textures"][texIter][0]
  //         let tdOffset = parseInt(
  //           textureslist[listIter]["textures"][texIter][1].toString()
  //         )
  //         let tdSize = parseInt(
  //           textureslist[listIter]["textures"][texIter][2].toString()
  //         )
  //         let tdUvrect = textureslist[listIter]["textures"][texIter][3]
  //         let tdImgRect = textureslist[listIter]["textures"][texIter][4]

  //         let imgBufferHead = Buffer.from(
  //           ifsBuffer.buffer.slice(
  //             manifest_end + tdOffset,
  //             manifest_end + tdOffset + 8
  //           )
  //         )
  ...
  //                   decompressed.push(
  //                     decompressed[decompressed.length + -position]
  //                   )
  //                 }
  //               }
  //             }
  ...
  //             decompressed[decCtr + 2] = [
  //               decompressed[decCtr],
  //               (decompressed[decCtr] = decompressed[decCtr + 2]),
  //             ][0]
  //           }

  //           let pngf = new PNG({
  //             width:
  //               Math.floor(tdImgRect[1] / 2) - Math.floor(tdImgRect[0] / 2),
  //             height:
  //               Math.floor(tdImgRect[3] / 2) - Math.floor(tdImgRect[2] / 2),
  //             bitDepth: 8,
  //             colorType: 6,
  //             inputHasAlpha: true,
  //           })
  ...
  //           runErrors.push(
  //             "[ifs] decompression mismatch for " +
  //               textureslist[listIter]["file"] +
  //               "/" +
  //               tdFileName
  //           )
  //         }
  ...
  let courseData = JSON.parse(
    U.DecodeString(
      await IO.ReadFile("webui/asset/json/course_data.json"),
      "utf8"
    )
  )
  ...
  if (courseDataUpdateSuccess === false)
    runErrors.push("[course_data] Update unsuccessful.")

  await IO.WriteFile(
    "webui/asset/json/course_data.json",
    JSON.stringify(courseData, null, 4)
  )


  try{
    const files = await recursive(U.GetConfig("sdvx_eg_root_dir") + "/data/music");
    const distDir = "plugins/sdvx_dev@asphyxia/webui/asset/jacket/"
    const filteredFiles = files.filter(
    (file) => path.extname(file) === ".png" && file.includes("jk_") && !file.includes("_b") && !file.includes("_s")
  );

  await fsp.mkdir(distDir, { recursive: true });
  for (const file of filteredFiles) {
    try{
      await fsp.copyFile(file, distDir + path.basename(file))
    }catch (err) {
      console.log("continuing", err);
    }
  }

  console.log("added jacket files at", distDir)
  
  // const folder = folders.filter(dir => dir.isDirectory() && dir.normalize.startsWith(folderId + "_")).map(dirent => dirent.name)
  // .sort()[0]; // Take the first match
...
export const getRivalScores = async (
  data: { rivalId: string; refid: string },
  send: WebUISend
) => {
  let rival = await DB.FindOne<Rival>(data.refid, {
    collection: "rival",
    refid: data.rivalId,
  })
  send.json({
    ...
    rivalScores: await DB.Find<MusicRecord>(rival.refid, {
      collection: "music",
    }),
  })
}
...
export const addRival = async (
  ...
  let checkMutual =
    (await DB.Count<Rival>(data.rivalId, {
      collection: "rival",
      refid: data.refid,
    })) > 0
  if (
    (await DB.Count<Rival>(data.refid, {
      collection: "rival",
      refid: data.rivalId,
    })) === 0
  ) {
...
export const preGeneRoll = async (
  data: { set: number; refid: string; items: [] },
  send: WebUISend
) => {
  let itemId = {
    crew: 11,
  ...
  if (preGeneSet != undefined) {
    while (!finishedRolling) {
      ...
      for (const probIndex in preGeneSet.probability) {
        ...
        if (parseInt(probIndex) === preGeneSet.probability.length - 1)
          rollWhat = parseInt(probIndex)
      }

      let unobtainedItems = preGeneSet.items[
        Object.keys(preGeneSet.items)[rollWhat]
      ].map((x) => x)
      data.items.forEach((item) => {
        ...
        if (
          item["type"] == itemId[Object.keys(preGeneSet.items)[rollWhat]] &&
          preGeneSet.items[Object.keys(preGeneSet.items)[rollWhat]].includes(
            checkId
          )
        ) {
      ...
      if (unobtainedItems.length > 0) {
        ...
        console.log(
          "Rolled item id: " +
            unobtainedItems[randomItemIndex] +
            " | item type: " +
            itemId[Object.keys(preGeneSet.items)[rollWhat]]
        )
        if (itemId[Object.keys(preGeneSet.items)[rollWhat]] == 17) {
          for (
            let stampID = unobtainedItems[randomItemIndex] * 4 - 3;
            stampID <= unobtainedItems[randomItemIndex] * 4;
            stampID++
          ) {
            DB.Upsert(
              data.refid,
        ...
        let finalItemType =
          Object.keys(preGeneSet.items)[rollWhat] === "subbg"
            ? "bg"
            : Object.keys(preGeneSet.items)[rollWhat]
...
export const preGeneReward = async (
  data: { reward: []; refid: string },
  send: WebUISend
) => {
  let reward = Object.values(data.reward)
  let rewardItem = await DB.Find<Item>(data.refid, {
    collection: "item",
    type: reward[0],
    id: reward[1],
    param: reward[2],
  ...
  if (rewardItem.length === 0) {
    DB.Upsert(
      data.refid,
      { collection: "item", type: reward[0], id: reward[1] },
      { $set: { param: reward[2] } }
    )
    send.json({
      received: true,
...
export const manageEvents = async (data: { eventConfig: {} }) => {
  IO.WriteFile(
    "webui/asset/config/events.json",
    JSON.stringify(data.eventConfig, null, 4)
  )
}

export const manageStartupFlags = async (data: { flagConfig: {} }) => {
  IO.WriteFile(
    "webui/asset/config/flags.json",
    JSON.stringify(data.flagConfig, null, 4)
  )
}
