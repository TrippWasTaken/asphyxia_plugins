  ...
  try {
    ...
    switch (info.method) {
      ...
      case 'sv6_common': {
        ...
        if (IO.Exists('webui/asset/config/flags.json')) {
          let bufFlagConfig = await IO.ReadFile(
          ...
          for (const flagIter in flagConfig) {
            if (flagConfig[flagIter]['toggle'])
              events.push(flagConfig[flagIter]['str']);
          }
        }
        courses = COURSES6;
    ...
    if (U.GetConfig('unlock_all_songs')) {
      ...
      for (let i = 0; i <= songNum; i++) {
        var foundSongIndex = mdb.mdb.music
          .map(function (x) {
            return x['@id'];
          })
          .indexOf(i.toString());
        if (foundSongIndex != -1) {
          ...
          if (gameVersion === 6 || gameVersion === -6) {
            if (
              'distribution_date' in songData['info'] &&
              parseInt(songData['info']['distribution_date']['#text']) >
                currentYMDDate
            ) {
              ...
              if (songData.info.version['#text'] === '6') {
                ...
                if (LICENSED_SONGS6.includes(i.toString())) limitedNo += 1;
                  ...
                  VALKYRIE_SONGS.includes(i.toString()) &&
                  info.model.split(':')[2].match(/^(G|H)$/g) == null
                )
                  limitedNo -= 1;
    ...
    if (U.GetConfig('use_information')) {
      ...
      const currentTime =
        parseInt((time.getTime() / 100000) as unknown as string) * 100;
      extend.push({
        id: 1,
        type: 1,
        params: [
    ...
    if (IO.Exists('webui/asset/config/events.json')) {
      ...
      for (const eventIter in eventData['events']) {
        ...
        if (
          eventData['events'][eventIter]['type'] === 'stamp' &&
          eventConfig[eventData['events'][eventIter]['id']] !== undefined &&
          ...
          if (stmpEvntInfo['type'] === 'select') {
            extend.push({
              ...
              params: [
                9,
                stmpEvntInfo['info']['textstampval'] !== undefined
                  ? stmpEvntInfo['info']['textstampval']
                  : 0,
                0,
                0,
                0,
          ...
          eventData['events'][eventIter]['type'] === 'completestamp' &&
          eventConfig[eventData['events'][eventIter]['id']] !== undefined &&
          eventConfig[eventData['events'][eventIter]['id']]['toggle']
        ) {
          extend.push({
          ...
          eventData['events'][eventIter]['type'] === 'tama' &&
          eventConfig[eventData['events'][eventIter]['id']] !== undefined &&
          eventConfig[eventData['events'][eventIter]['id']]['toggle']
        ) {
    ...
    if (U.GetConfig('use_asphyxia_gameover')) {
      ...
      switch (pattern) {
        case 1: {
          var chara1 =
            leftCharater[getRandomIntInclusive(0, leftCharater.length - 1)];
          var chara2 =
            rightCharater[getRandomIntInclusive(0, rightCharater.length - 1)];
          var chara3 =
            leftCharater[getRandomIntInclusive(0, leftCharater.length - 1)];
          charaString +=
            'chara01/' + chara1 + ' chara02/' + chara2 + ' chara01/' + chara3;
          break;
        }
        case 2: {
          var chara1 =
          ...
          var chara3 =
            rightCharater[getRandomIntInclusive(0, rightCharater.length - 1)];
          charaString +=
            'chara02/' + chara1 + ' chara01/' + chara2 + ' chara02/' + chara3;
          break;
        }
        case 3: {
          var chara1 =
            rightCharater[getRandomIntInclusive(0, rightCharater.length - 1)];
          var chara2 =
            middleCharater[getRandomIntInclusive(0, middleCharater.length - 1)];
          var chara3 =
            leftCharater[getRandomIntInclusive(0, leftCharater.length - 1)];
          charaString +=
            'chara02/' + chara1 + ' chara03/' + chara2 + ' chara01/' + chara3;
          break;
        }
        case 4: {
          var chara1 =
          ...
          var chara3 =
            rightCharater[getRandomIntInclusive(0, rightCharater.length - 1)];
          charaString +=
            'chara01/' + chara1 + ' chara03/' + chara2 + ' chara02/' + chara3;
          break;
        }
      }

      if (Math.abs(getVersion(info)) == 6) {
        //Due to older version misses newer characters, not supported on older versions
        extend.push({
          id: 3,
          type: 1,
          params: [
    ...
    if (currentDate.substring(0, 5) === '2/14/')
      events.push('VALENTINES_DAY_2024');
    ...
    if (
      [
        ...
        '10/30/',
        '10/31/',
      ].includes(currentDate.substring(0, 6))
    )
      events.push('HALLOWEEN_EVENT');
    if (['12/24/', '12/25/', '12/26/'].includes(currentDate.substring(0, 6)))
      events.push('MERRY_CHRISTMAS_2023');
    ...
    send.object(
      {
        ...
        arena:
          arena_szn !== 'None'
            ? {
                ...
                rank_match_target: K.ITEM(
                  's32',
                  ARENA[arena_szn].details.rank_match_target
                ),
        ...
        skill_course: {
          info: courses.reduce((acc, s) => {
            ...
            if (
              (info.model.split(':')[2] === 'G' ||
                info.model.split(':')[2] === 'H') &&
              s.hasGod !== undefined &&
              s.hasGod === 1
            ) {
